...
### 实现特点
- **Streamlit原生列布局**：使用 `st.columns([2, 1, 1, 1, 1])` 创建5列横向布局
- **渐变背景**：使用 CSS 渐变色突出主题色
- **圆角边框**：12px 圆角，现代化设计
- **响应式布局**：自适应容器宽度
- **统一配色**：使用项目主题色（accent color #10a37f）
- **混合渲染**：Streamlit组件 + 内联CSS样式，确保完美显示
...
### 布局结构
采用5列布局 `st.columns([2, 1, 1, 1, 1])`：
- **第1列（宽度2）**：显示作者名称和时间信息
- **第2-5列（各宽度1）**：分别显示评论、点赞、分享、收藏统计
  - 每列包含：图标 + 数值 + 标签
...
3. **第三次问题**：改用Flexbox布局（`display: flex`），但仍然显示原始HTML代码而非渲染后的效果
4. **最终解决方案**：使用Streamlit原生的 `st.columns()` 组件创建横向布局，配合少量HTML美化，确保完美渲染
